...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Workspace: live/cached runtime snapshots keyed by session id.
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            self._bind_active_session_agent(effective_task_id, agent)
            try:
                result = agent.run_conversation(
                    user_message=user_message,
                    conversation_history=conversation_history,
                ...
                _eff_sid = getattr(agent, "session_id", effective_task_id)
                if isinstance(_eff_sid, str) and _eff_sid:
                    result["session_id"] = _eff_sid
                return result, usage
            finally:
                self._store_session_runtime_snapshot(effective_task_id, agent)
                self._unbind_active_session_agent(effective_task_id, agent)
